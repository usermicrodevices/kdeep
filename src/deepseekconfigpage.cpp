    ...
    , m_config(nullptr)
{
    ...
    QPushButton *refreshModelsBtn = new QPushButton(i18n("Refresh Models"), this);
    modelLayout->addWidget(refreshModelsBtn);
    layout->addRow(i18n("Model:"), modelLayout);

    connect(refreshModelsBtn, &QPushButton::clicked, this, &DeepSeekConfigPage::refreshModels);
    ...
    m_useGpt4allCheck = new QCheckBox(i18n("Use GPT4All API Server (local, no API key and model needed)"), this);
...
void DeepSeekConfigPage::loadSettings()
{
    ...
    bool useGpt4all = m_config->readEntry("useGpt4all", false);
    m_useGpt4allCheck->setChecked(useGpt4all);
    m_apiKeyEdit->setEnabled(!useGpt4all);
}
...
void DeepSeekConfigPage::refreshModels()
{
